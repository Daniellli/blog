		...
		function clock(){	
		   ...
		   function toTarget(){
			   ...
			   var value = $("#phone").val();
			   if(refer == ""){
				   location.href =  "https://passport2.chaoxing.com/wlogin?inputvalue="+value;
			   }else{
				   location.href =  "https://passport2.chaoxing.com/wlogin?inputvalue="+value+"&refer="+refer;
			   }
		   }
			...
			if(value == ""){
				...
				if(unameIsPhone(value)){
					if(!isValPhone(value)){
						...
						$.ajax({ 
						 url: "/num/phonecode?phone="+value+"&needcode=false",
						 type:"get",
		...
		function isValPhone(temp){
	 	$.ajax({ 
			url: "/api/isExistPhoe?phone="+temp,
			type:"get",
			success: function(date){
				 var myjson=eval("("+date+")");
